                        ...
                        for residue in chain:
                            ...
                            for atom_name in needed_atoms:
                                if atom_name in residue:
                                    ...
                                    line = (
                                        f"ATOM  {atom_num:5d} {atom_name:<4s} "
                                        f"{resname:3s} {chain_id:1s}{resnum:4d}    "
...
def predict_rna_structures_drfold2(
        ...
        with open(file_path, "r") as f:
            ...
            for j in range(1, len(sequence) + 1):
                if j in residue_map:
                    ...
                    logger.warning(
                        f"Residue {j} not found in {pdb_file} for {target_id}"
                    )
    ...
    while len(predictions) < 5:
        predictions.append(
            predictions[-1]
            if predictions
            else [(0.0, 0.0, 0.0) for _ in range(len(sequence))]
        )
