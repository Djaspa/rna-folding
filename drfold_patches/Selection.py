...
# from scipy.optimize import fmin_bfgs, fmin_cg, fmin_l_bfgs_b, minimize
...
class Structure:
    ...
    def compute_pccp_energy(self, coor):
        ...
        return self._cubic_torsion_energy(
            dia, self.pccp_coe, self.pccp_x, "weight_pccp", 36
        )
    ...
    def compute_cnnc_energy(self, coor):
        ...
        return self._cubic_torsion_energy(
            dia, self.cnnc_coe, self.cnnc_x, "weight_cnnc", 36
        )
    ...
    def compute_pnnp_energy(self, coor):
        ...
        return self._cubic_torsion_energy(
            dia, self.pnnp_coe, self.pnnp_x, "weight_pnnp", 36
        )
    ...
    def compute_pcc_energy(self, coor):
        ...
        p = coor[:, 1]
        c = coor[:, 2]
        ang = operations.angle(p[self.pcci], c[self.pcci], c[self.pccj])
    ...
    def compute_fape_energy_fromcoor(self, coor, ep=1e-6, epmax=100):
        ...
        p_rot, px_mean = operations.Kabsch_rigid(
            self.basex, coor[:, 0], coor[:, 1], coor[:, 2]
        )
        pred_x2 = coor[:, None, :, :] - px_mean[None, :, None, :]  # Lx Lrot N , 3
        pred_x2 = torch.einsum(
            "ijnd,jde->ijne", pred_x2, p_rot.transpose(-1, -2)
    ...
    def energy(self, rama):
        ...
        E_cc = (
            self.compute_cc_energy(coor) / len(self.geofiles)
            if self.config["weight_cc"] > 0
            else 0
        )
        E_pp = (
            self.compute_pp_energy(coor) / len(self.geofiles)
            if self.config["weight_pp"] > 0
            else 0
        )
        E_nn = (
            self.compute_nn_energy(coor) / len(self.geofiles)
            if self.config["weight_nn"] > 0
            else 0
        )
        E_pccp = (
            self.compute_pccp_energy(coor) / len(self.geofiles)
            if self.config["weight_pccp"] > 0
            else 0
        )
        E_cnnc = (
            self.compute_cnnc_energy(coor) / len(self.geofiles)
            if self.config["weight_cnnc"] > 0
            else 0
        )
        E_pnnp = (
            self.compute_pnnp_energy(coor) / len(self.geofiles)
            if self.config["weight_pnnp"] > 0
            else 0
        )
        E_vdw = (
            self.compute_full_clash(coor, other_coor, side_coor)
            if self.config["weight_vdw"] > 0
            else 0
        )
        E_fape = (
            self.compute_fape_energy_fromquat(rama[:, 9:], coor) / len(self.geofiles)
            if self.config["weight_fape"] > 0
            else 0
        )
        E_bond = (
            self.compute_bond_energy(coor, other_coor)
    ...
    def energy_from_coor(self, coor):
        ...
        E_fape = (
            (self.compute_fape_energy_fromcoor(coor) / len(self.geofiles))
            if self.config["weight_fape"] > 0
            else 0
        )
    ...
    def foldning(self):
        ...
        minirama = None  # noqa: F841

        ilter = self.init_ret
        selected_ret = self.geofiles[ilter]  # noqa: F841
        try:
            rama = self.init_quat(ilter).data.numpy()
            self.config = readconfig(
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "lib", "vdw.json"
                )
            )
            rama = fmin_l_bfgs_b(
                func=self.obj_func_np,
                x0=rama,
                fprime=self.obj_func_grad_np,
                iprint=10,
                maxfun=100,
            )[0]
            rama = rama.flatten()
        ...
        for i in range(3):
            line_min = lbfgs_rosetta.ArmijoLineMinimization(
                self.obj_func_np, self.obj_func_grad_np, True, len(rama), 120
            )
            lbfgs_opt = lbfgs_rosetta.lbfgs(self.obj_func_np, self.obj_func_grad_np)
            rama = lbfgs_opt.run(
                rama,
                256,
                lbfgs_rosetta.absolute_converge_test,
                line_min,
                8000,
                self.obj_func_np,
                self.obj_func_grad_np,
                1e-9,
            )
        newrama = rama + 0.0
        ...
        if current_energy < minenergy:
            ...
            self.outpdb(
                newrama, self.saveprefix + ".pdb", energystr=str(current_energy)
            )
    ...
    def outpdb(self, rama, savefile, start=0, end=10000, energystr=""):
        ...
        side_coor_NP = a2b.quat2b(  # noqa: F841
            self.sidex, torch.cat([shaped_rama[:, :9], coor[:, -1]], dim=-1)
        ).data.numpy()
