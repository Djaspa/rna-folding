...
def run_cmd(cmd, description):
    ...
    process = Popen(
        cmd, shell=True, stdout=PIPE, stderr=STDOUT, universal_newlines=True, bufsize=1
    )
...
if not os.path.isfile(ret_dir + "/done"):
    ...
    print("[DRfold2] No previous predictions found, will generate e2e and geo files")
    ...
    print(
        f"[DRfold2] Found previous predictions in {ret_dir}, using existing e2e "
        f"and geo files"
    )
...
optpython = os.path.join(
    exp_dir, "PotentialFold", "Optimization.py"
)  # Optimization script
clupy = os.path.join(exp_dir, "PotentialFold", "Clust.py")  # Clustering script
arena = os.path.join(
    exp_dir, "Arena", "Arena"
)  # Arena executable for structure refinement


# Set up initial save prefixes for optimization and selection
optsaveprefix = os.path.join(cso_dir, "opt_0")
save_prefix = os.path.join(cso_dir, "sel_0")
...
print("[DRfold2] Found {len(rets)} return files for selection")
print("[DRfold2] Using selection config: {config_sel}")
print("[DRfold2] Output prefix: {save_prefix}")
...
cmd = (
    f"python {optpython} {fastafile} {optsaveprefix} {ret_dir} {save_prefix} "
    f"{foldconfig}"
)
...
if pclu:
    ...
    for i in range(1, len(lines)):
        ...
        cmd = (
            f"python {optpython} {fastafile} {optsaveprefix} {ret_dir} "
            f"{save_prefix} {foldconfig}"
        )
        if af3file and os.path.exists(af3file):
            cmd += f" {af3file}"
